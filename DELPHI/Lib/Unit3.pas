...
begin
  ...
  begin
    ShowMessage('No such method ID');/// XM
    meth:=nil;
  end;
  end;
  ...
  end else ShowMessage('No such method ID');
...
var
  ...
  TempBit1,TempBit2:TBitmap;
  Res:array[0..4] of real;
  result:vec6;
  preloaded:Boolean;
begin

  HashL:=nil;  //hide warning
  HashR:=nil;
  NHashL:=nil;
  NHashR:=nil;
  meth:=nil;// should unhide
  ...
  begin
    ...
    begin
        ...
        LoadJpegToBitmap(Self.GoodFilePairs[i,0],TempBit1);
        bit1.Canvas.StretchDraw(Rect(0,0,640,360),TempBit1);
        LoadJpegToBitmap(Self.GoodFilePairs[i,1],TempBit2);
        bit2.Canvas.StretchDraw(Rect(0,0,640,360),TempBit2);
        picl:=Api.FromPicToArr(bit1,Api.Sett);
        picR:=Api.FromPicToArr(bit2,Api.Sett);
        NPicL:=CopyPicArr(Picl);
        NPicR:=CopyPicArr(PicR);
  ...
  begin
    ...
    begin
      tempbit1:=tbitmap.create;
      LoadJpegToBitmap(Self.BadFilePairs[i,0],TempBit1);
      bit1.Canvas.StretchDraw(Rect(0,0,640,360),TempBit1);

      tempbit1:=tbitmap.create;
      LoadJpegToBitmap(Self.BadFilePairs[i,1],TempBit2);
      bit2.Canvas.StretchDraw(Rect(0,0,640,360),TempBit2);
      picl:=Api.FromPicToArr(bit1,Api.Sett);
      picR:=Api.FromPicToArr(bit2,Api.Sett);
      NPicL:=CopyPicArr(Picl);
